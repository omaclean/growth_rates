...
weeks_to_check=1
start_date=max(as.numeric(new_tab_in$sample_date))
start_date=as.numeric(as.Date('2021-11-20',origin='1970-01-01'))
days_to_analyse=100
...
for(week_try in 0:(weeks_to_check-1)){
  date_try=start_date-week_try*7
  ...
  muts=unique(muts)
  muts=unique(c('Y145H','A222V',"V1264L",muts))
  ...
  hits_plot_name=muts_to_plot=hits=unique(c(#'R346K','V213G','N501Y',"A701V","N764K",
                                           hits))
  ...
  for(i in 1:length(muts_to_plot)){
    ...
    for(j in 1:length(other_muts)){
      ...
      sum_other=sum(new_tab2[,other_muts[j]])
    }
    if(any(coex>0.97*sum(new_tab2[,muts_to_plot[i]])&
           sum(new_tab2[,muts_to_plot[i]])<0.9*sum_other)){
      muts_to_plot_rel[i]=names(coex)[which(coex==max(coex))[1]]
    }
  }
  ...
  for(i in 1:length(muts_to_plot)){
    ...
    dat_print=rbind(dat_print,c(date_try,muts_to_plot[i],round(summary(mod)$coef[1,1],3),round(confint2(mod)[1,1],3),
                                round(confint2(mod)[1,2],3),c(muts_to_plot_rel[i])))
...
colnames(dat_print)=c('week','mutation','growth','CI1','CI2','parent_mut')
ylim_plot=as.numeric(range(dat_print[,5]))
xlim_plot=as.numeric(c(max(as.numeric(new_tab_in$sample_date))-weeks_to_check*7,
            max(as.numeric(new_tab_in$sample_date))))
dat_print2=dat_print
library(plotrix)
dat_print=as.data.frame(dat_print)
dat_print[,1]=as.Date(dat_print[,1],origin='1970-01-01')
dat_print[,3]=as.numeric(dat_print[,3])
dat_print[,4]=as.numeric(dat_print[,4])
dat_print[,5]=as.numeric(dat_print[,5])
par(mfrow=c(ceiling(length(unique(dat_print[,2]))/3),3),
    mar=c(3,3,1,1))
save(dat_print,file='C:/Users/Oscar/Documents/test.muts.Rdata')
for(mut_hits in unique(dat_print[,2])){
  plotCI(dat_print[dat_print[,2]==mut_hits,1],
           dat_print[dat_print[,2]==mut_hits,3],
           liw=dat_print[dat_print[,2]==mut_hits,4],
           uiw=dat_print[dat_print[,2]==mut_hits,5],
           xlim=xlim_plot,ylim=ylim_plot,
           main=paste(mut_hits,'on',paste(unique(dat_print[dat_print[,2]==mut_hits,6]),
                                         collapse=' & ')))
